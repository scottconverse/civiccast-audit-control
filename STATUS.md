# Program status ledger — native-Windows migration

Live state of the charter §8 sequence. This file records outcomes with evidence
...
**Last updated:** 2026-07-17 (coder, overnight autonomous session — owner
...
| 3 | Superseding ADR + claims-evidence rule | **SPEC DRAFTING** (tonight) | ADR draft + claims-verifier spec to be committed for auditor design review |
| 4 | Dual-runtime exclusion guard | **SPEC DRAFTING** (tonight) | Implementation follows as its own slice; guard required before any side-by-side install |
| 5 | Native supervisor + media-worker decision gate | **GATE HALF: PASSED** — supervisor build remains | Decision gate PASSED 2026-07-17: unmodified engine.py/worker.py natively — 4 hot swaps clean TS, 0.00s hard-kill reap + relaunch, 3 concurrent channels (civiccast `.agent-runs/native-windows/spike-decision-gate/`, commit `3b4b2451`). **Python media worker RETAINED; Rust gstreamer-rs port CLOSED** (recorded fallback). **Session-0 spike PASSED** 2026-07-17: engine as LocalSystem service, started 12s after boot, 6m14s / ~230MB broadcast before first interactive logon (Security 4624 primary source), 60 swaps, clean teardown (civiccast `.agent-runs/native-windows/spike-session0/`, commit `115531df`). Remaining §7 slice items: supervisor subsystem, packaging closure, two-pristine-machine proof |
...
GStreamer native closure (50/51 factories + caption round-trip) · PyGObject via
pip (`gstreamer-bundle==1.28.5`) · engine behavior native (switching, recovery,
multichannel) · session-0 pre-login service broadcast. What remains is
