...
| 3 | Superseding ADR + claims-evidence rule | **ws3 IN AUDIT LOOP (round 3)** | Implementation on PR #293 (claude/ws3-claims-evidence): round 1 CHANGES_REQUIRED at `8b80967e` (a10cc384 - the rule caught its own program's cross-branch overclaim on first run); round 2 CHANGES_REQUIRED at `e87b4724` (adfa90bc); round 3 CHANGES_REQUIRED at `dd34eb37` (6eadc062); round 4 CHANGES_REQUIRED at `ab39e927` (30a636d8: CC-WS3-006 input binding CLOSED; NEW CC-WS3-008 Major - the base-merge's backup/restore prose resolved through the capture-only marker, a proof-association false pass; CC-WS3-007 Minor batched); round 5 in build (register ws2-postgres-restore-drill entry + per-block capability coverage in the D1 scanner + parser/schema parity). ADR-0021 auditor half PASS - OWNER MERGE PENDING; claims spec v13 is the contract; trust-root pin + authority-record-v1 await owner acceptance (external claims correctly PENDING_OWNER_ONLY until then) |
| 4 | Dual-runtime exclusion guard | **ws4 ROUND-1 AUDIT REQUESTED** | Built through 3 internal adversarial rounds (8 findings closed red-first pre-commit); head `5d588be4` on PR #295, CI green run 29594844868 incl. the NEW windows-native producer job (floor 125, zero skips); WSL-side patch delivered as evidence artifact only (owner-routed, co-install prerequisite). Round-1 audit request sent |
